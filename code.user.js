...
(function ($, async) {
    ...
    if (currentPage == PAGE_INVENTORY) {
        ...
        function updateTotals() {
            ...
            if (totalPriceWithFeesOnMarket > 0) {
                totals.innerHTML += `<div><strong>累计上架物品总价为 ${formatPrice(totalPriceWithFeesOnMarket)}，你将会获得 ${formatPrice(totalPriceWithoutFeesOnMarket)}。</strong></div>`;
            }
            if (totalScrap > 0) {
        ...
        const sellQueue = async.queue(
            (task, next) => {
                ...
                if (getSettingWithDefault(SETTING_PRICE_MIN_LIST_PRICE) * 100 >= market.getPriceIncludingFees(task.sellPrice)) {
                    logDOM(`由于价格忽略设置，${padLeft} - ${itemNameWithAmount} 未能上架。`);
        ...
        async function updateInventorySelection(selectedItem) {
            ...
            const isBoosterPack = selectedItem.name.toLowerCase().endsWith('booster pack') || selectedItem.name.endsWith('补充包');
            if (isBoosterPack) {
            ...
            market.getOrderBook(
                ...
                (err, orderbook) => {
                    if (err) {
                        logConsole(`无法获取 ${selectedItem.name || selectedItem.description.name} 的订单簿`);
                        return;
                    }
                    ...
                    const groupMain = $(`<div id="listings_group">
                        <div>
                            ...
                            <table class="market_commodity_orders_table"><tr><th align="right">价格</th><th align="right">数量</th></tr>${sellRows}</table>
                        </div>
                        <div>
                            <div id="listings_buy">购买</div>
                            <table class="market_commodity_orders_table"><tr><th align="right">价格</th><th align="right">数量</th></tr>${buyRows}</table>
                        </div>
                    </div>`);
                ...
                (errorPriceHistory, history, cachedHistory) => {
                    ...
                    market.getOrderBook(
                        ...
                        (errorOrderBook, orderbook, cachedListings) => {
                            if (errorOrderBook) {
                                logConsole(`无法获取 ${game_name} 的订单簿`);
                            ...
                            $(
                                ...
                                listingUI
                            ).append(` ➤ <span title="这可能是当前最高买价。">${highestBuyOrderPrice
                                }</span>`);
    ...
    function initializeMarketHistoryUI() {
        ...
        $('#tabContentsMyMarketHistory').observe('childlist subtree', () => {
            ...
            if (controlsDiv.length > 0 && $('#see_page_jump').length === 0) {
                ...
                const btn = $('<span class="btn_green_white_innerfade btn_small" style="cursor: pointer;"><span>跳转</span></span>');
        ...
        const price_options = $(`<div id="see_settings_modal">
            <div>
                ...
                <select id="${SETTING_PRICE_ALGORITHM}">
					...
					<option value="4" ${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 4 ? 'selected="selected"' : ''}>仅历史均价</option>
                </select>
            </div>
    ...
    injectCss(`
        ...
        .market_commodity_orders_table th, .market_commodity_orders_table td { min-width: 80px; }
